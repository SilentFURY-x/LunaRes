...
export default function ProvenancePanel({ product }: ProvenancePanelProps) {
  return (
    <div className="border border-crater p-3">
      <h4 className="text-xs text-regolith/50 mb-2 font-display">Provenance</h4>
      ...
      {product.downstream_delta && (
        <div className="mt-3 border-t border-crater pt-2">
          <h4 className="text-xs text-regolith/50 mb-1 font-display">
            Downstream Task Delta
          </h4>
          <p className="text-sm text-regolith">
            {product.downstream_delta.description}
          </p>
          <p className="text-xs text-regolith/60 mt-1">
...
function ProvenanceRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between">
      <dt className="text-regolith/50">{label}</dt>
      <dd className="font-mono text-regolith">{value}</dd>
    </div>
  );
}
